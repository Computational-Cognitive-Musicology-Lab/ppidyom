...
see [README.md](https://ppidyom.ccml.gtcmt.gatech.edu/README.md).
...
Open `ppidyom.Rproj` in RStudio — this sets the working directory and
activates the project environment automatically.
...
`load_all()` sources every file in `R/` into the current session without
...
Performs a full build and install into your R library. Use this when you
...
Compares ppidyom against ground-truth IDyOM (Common Lisp) output from a
pre-generated fixture CSV. All tests use `N = 3`, `idyom_base = TRUE`,
...
IDyOM’s AX escape is invoked as `:x` (not `:ax` — see §6 of
...
The test file `tests/testthat/test-idyom-comparison.R` validates ppidyom
against real IDyOM (Common Lisp) output. This is a three-way check for
STM:
...
The comparison requires a pre-generated reference CSV. You only need to
...
script. No R code changes are needed — the test file reads all configs
directly from the CSV.

### 4.5 What the Dockerfile installs

`inst/validation/Dockerfile` installs: - `sbcl` — Steel Bank Common Lisp
(the Lisp compiler IDyOM runs on) - `quicklisp` — Common Lisp package
manager (downloads IDyOM automatically) - `idyom` — Pearce’s IDyOM
...
**Count table** (output of `count_tables`, input to `ppm_interpolated` /
`ppm_backoff`):
...
During training, counts are stored as `new.env(hash=TRUE)` objects — one
environment per order. Each key is a `context_id` string; each value is
a named integer vector `c(A=3, B=1, ...)` mapping symbols to Ce counts.
These are converted to data.tables at the end of `count_tables`. Only
